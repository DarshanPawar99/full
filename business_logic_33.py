...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            calculated_buying_price = safe_get_value(row, 'buying price ai') / safe_get_value(row, 'vendor gst')
            ...
            calculated_selling_amount_ai = ((safe_get_value(row, 'selling pax') * safe_get_value(row, 'selling price')) * safe_get_value(row, 'client gst'))+ safe_get_value(row, 'selling transportation')
            check_mismatch(row, index, 'selling amount ai', calculated_selling_amount_ai, mismatched_data)
...
def business_logic_33(df_selected_month, analysis_df, months_to_include):
