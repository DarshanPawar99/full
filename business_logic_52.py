...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            calculated_buying_amt_bc = safe_get_value(row, 'net sale food vendor') * safe_get_value(row, 'sq commision %')
            check_mismatch(row, index, 'commission value', calculated_buying_amt_bc, mismatched_data)

            calculated_buying_amt = safe_get_value(row, 'net sale food vendor') - (safe_get_value(row, 'commission value'))
...
def analysis_data(analysis_df, months_to_include):
    ...
    columns_to_fill = ['net sale food vendor', 'commission value', 'buying amt ai']
    analysis_df[columns_to_fill] = analysis_df[columns_to_fill].fillna(0)

    analysis_df['total_gmv'] = analysis_df['net sale food vendor']
    analysis_df['total_buying'] = analysis_df['buying amt ai']
    # Group data by month and calculate sums
    metrics = ['commission value', 'total_gmv', 'total_buying']
    ...
    if len(summary_df.columns) > 1:
        ...
        rows_to_calculate = ['commission value', 'total_gmv', 'total_buying']
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_selling_amt_regular = df_filtered['net sale food vendor'].sum()
    sum_commission = df_filtered['commission value'].sum()
...
def business_logic_52(df_selected_month, analysis_df, months_to_include):
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                ...
                'regular selling amount': [full_data['net sale food vendor'].sum()]
            })
...
def dump_data(df_filtered, month, dump_file_path):
    dump_mapping = {
        ...
        'selling amount': 'net sale food vendor',
