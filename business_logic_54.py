...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_pax = max(safe_get_value(row, 'ordered pax/vendor mg'), safe_get_value(row, 'buying actual consumption'))
            check_mismatch(row, index, 'buying pax', calculated_buying_pax, mismatched_data)

            calculated_selling_pax = max(safe_get_value(row, 'client mg/pre order'), safe_get_value(row, 'selling actual consumption'))
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    st.write("Buying on Higest of Ordered Pax, Buying Actual Consumption")
    st.write("Selling  on Higest of Client MG, Selling Actual Consumption")
    st.markdown("---")
...
def business_logic_54(df_selected_month, analysis_df, months_to_include):
