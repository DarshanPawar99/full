...
def analysis_data(analysis_df, months_to_include):
    ...
    metrics = ['total_gmv', 'total_buying', 'commission', 'karbon', 'net_revenue']
    ...
    if len(summary_df.columns) > 1:
        ...
        rows_to_calculate = ['total_gmv', 'total_buying', 'commission', 'karbon', 'net_revenue']
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df):
    st.write("Buying And Selling Is neither on MG nor on Actual Consumpation")
    st.markdown("---")
...
def business_logic_10(df_selected_month, analysis_df, months_to_include):
