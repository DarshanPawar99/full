    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            if row['session'] == 'dinner':
                ...
                calculated_bt = 0
            check_mismatch(row, index, 'buying transportation', calculated_bt, mismatched_data)

            if row['session'] == 'dinner':
                calculated_st = 1000
            else:
                calculated_st = 0
            check_mismatch(row, index, 'selling transportation', calculated_st, mismatched_data)
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    ...
    st.write("Transportation for all Dinner Session") 
    st.markdown("---")
...
def business_logic_6(df_selected_month, analysis_df, months_to_include):
