...
def pivot_and_average_prices(df_filtered):
    pivot_df = df_filtered[['month','buying amount','selling amount','commission']]
    return pivot_df
...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            calculated_commission = safe_get_value(row, 'selling amount') - safe_get_value(row, 'buying amount')
...
def analysis_data(analysis_df, months_to_include):
    ...
    if len(summary_df.columns) > 1:
        ...
        rows_to_calculate = ['total_gmv','karbon','commission']
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_sams= df_filtered['buying amount'].sum()
    sum_selling_amt_regular = df_filtered['selling amount'].sum()

    valid_dates_df = df_filtered[(df_filtered['buying amount'] > 0) | (df_filtered['selling amount'] > 0)]
    number_of_days = valid_dates_df['date'].nunique()

    aggregated_data = {
        'Number of Days': number_of_days,
        'Total Sells' : sum_selling_amt_regular,
        'Karbon': sum_sams,
...
def business_logic_62(df_selected_month, analysis_df, months_to_include):
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                'sams': [full_data['buying amount'].sum()],
                'support on sells': [full_data['selling amount'].sum()]
            })
    ...
    try:
        ...
        for key_value, group in df_filtered.groupby('identifier'):
            ...
            pnl_df.loc[(pnl_df['identifier'] == key_value) & (pnl_df['month'] == month), 
                       ['sams', 'support on sells']] = pnl_data[[ 'sams','support on sells']].values
