...
def pivot_and_average_prices(df_filtered):
    ...
    pivot_df = df_filtered.pivot_table(
        index=['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type'],
        aggfunc='size'
...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_amt_ex = safe_get_value(row, 'buying amt ai') / safe_get_value(row, 'gst') 
            check_mismatch(row, index, 'buying amt ex gst', calculated_buying_amt_ex, mismatched_data)

            # Additional checks for 'cash received'
            if row['menu item'] == 'consumables':
                calculated_selling_amt = safe_get_value(row, 'buying amt ai') * 1.15
                check_mismatch(row, index, 'selling amount', calculated_selling_amt, mismatched_data)
            else:
                calculated_selling_amt = safe_get_value(row, 'selling amount')
                check_mismatch(row, index, 'selling amount', calculated_selling_amt, mismatched_data)
...
def business_logic_16(df_selected_month, analysis_df, months_to_include):
        ...
        for key_value, group in df_filtered.groupby('identifier'):
            ...
            pnl_df.loc[(pnl_df['identifier'] == key_value) & (pnl_df['month'] == month), 
                       [ 'regular buying amount', 'regular selling amount'
                        ]] = pnl_data[['regular buying amount', 'regular selling amount' 
                        ]].values
