...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            calculated_buying_amt_bc = safe_get_value(row, 'buying qty.') * safe_get_value(row, 'buying price')
            check_mismatch(row, index, 'buying amount (before comm)', calculated_buying_amt_bc, mismatched_data)

            calculated_buying_amt = safe_get_value(row, 'buying amount (before comm)') - (safe_get_value(row, 'buying amount (before comm)') * 0.1)
            check_mismatch(row, index, 'buying amt', calculated_buying_amt, mismatched_data)

            calculated_selling_amt = safe_get_value(row, 'selling qty') * safe_get_value(row, 'selling price')
            check_mismatch(row, index, 'selling amount', calculated_selling_amt, mismatched_data)

            calculated_buying_amt = safe_get_value(row, 'selling amount') - safe_get_value(row, 'buying amt')
            check_mismatch(row, index, 'commission', calculated_buying_amt, mismatched_data)
...
# Fill missing values with 0 for specified columns
    columns_to_fill = ['selling amount', 'commission']
    ...
    if len(summary_df.columns) > 1:
        ...
        rows_to_calculate = ['commission','total_gmv']
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_buying_amt_ai_regular= df_filtered['buying amt'].sum()
...
def display_dataframes(mismatched_data, aggregated_data, summary_df):
    st.write("Its ABM -MEAL in Airtel")
    st.markdown("---")
...
def business_logic_63(df_selected_month, analysis_df, months_to_include):
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                'regular buying amount': [full_data['buying amt'].sum()],
    ...
    try:
        ...
        for key_value, group in df_filtered.groupby('identifier'):
            ...
            pnl_df.loc[(pnl_df['identifier'] == key_value) & (pnl_df['month'] == month), 
                       ['regular buying amount', 'regular selling amount']] = pnl_data[['regular buying amount', 'regular selling amount']].values
...
def clear_pnl_data(p_and_l_file_path, month):
    ...
    try:
        ...
        columns_to_clear = [
            'regular buying amount', 'regular selling amount',]
