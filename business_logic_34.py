    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            if row['session'] == 'lunch':
                calculated_buying_pax = max(safe_get_value(row, 'client mg/pre order'), safe_get_value(row, 'ordered pax/vendor mg')) 
            elif row['session'] == 'dinner':
                calculated_buying_pax = safe_get_value(row, 'ordered pax/vendor mg')
            else: 
                calculated_buying_pax = "Invalid Session"
            ...
            calculated_cash_recived = (safe_get_value(row, 'actual consumption') * safe_get_value(row, 'employee contribution'))
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    st.write("Buying on Highest among Order MG, Actual Consumption for Lunch \n Buying on Vendor MG for Dinner")
...
def business_logic_34(df_selected_month, analysis_df, months_to_include):
