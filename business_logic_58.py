...
def analysis_data(analysis_df, months_to_include):
    ...
    analysis_df['regular_gmv'] = analysis_df.apply(lambda x: x['total btc sales ex'] if x['order type'] in regular_types else 0, axis=1)
    analysis_df['event_gmv'] = analysis_df.apply(lambda x: x['total btc sales ex'] if x['order type'] in event_types else 0, axis=1)

    # Calculate additional columns
    analysis_df['net_revenue'] = analysis_df['commission'] - analysis_df['amount']
    analysis_df['total_gmv'] = analysis_df['total btc sales ex']
    ...
    if len(summary_df.columns) > 1:
        ...
        rows_to_calculate = ['regular_gmv', 'event_gmv', 'commission', 'karbon', 'net_revenue', 'total_gmv']
...
def calculate_aggregated_values(df_filtered):
    ...
    valid_dates_df = df_filtered[(df_filtered['non veg meal coupon'] > 0) | (df_filtered['veg meal coupon'] > 0)]
...
def find_higher_buying(df_filtered):
    high_buying = []
    for index, row in df_filtered.iterrows():
        if (safe_get_value(row, 'vendor payout ai') > safe_get_value(row, 'total btc sales ex') > 0):
            high_buying.append({
                'Row': index + 3,
                'Date': row['date'],
                'Session': row['session'],
                ...
                'Buying Amount AI': row['vendor payout ai'],
                'Selling Amount': row['total btc sales ex'],
                'Remarks': row['remarks']
            })
    return high_buying
...
def display_dataframes(mismatched_data, karbon_expenses_data, aggregated_data, high_buying, summary_df):
    st.write("Recieve Sodexio Coupoun while serving meals")
    st.markdown("---")
...
def business_logic_58(df_selected_month, analysis_df, months_to_include):
...
def load_business_logic(df_filtered):
    try:
        ...
        for cost_centre, group in grouped:
            ...
            no_of_days = group[(group['non veg meal coupon'] > 0) | (group['veg meal coupon'] > 0)]

            pnl_data = pd.DataFrame({
                'days': [no_of_days['date'].nunique()],
                'regular buying amount': [regular_amt['vendor payout ai'].sum()],
                'regular selling amount': [regular_amt['total btc sales ex'].sum()],
                'event buying amount': [event_amt['vendor payout ai'].sum()],
                'event selling amount': [event_amt['total btc sales ex'].sum()],
    ...
    try:
        ...
        for key_value, group in df_filtered.groupby('identifier'):
            ...
            pnl_df.loc[(pnl_df['identifier'] == key_value) & (pnl_df['month'] == month), 
                       ['days','regular buying amount', 'regular selling amount','event buying amount', 'event selling amount', 'penalty on vendor', 'penalty on smartq', 'sams']] = pnl_data[['days' ,'regular buying amount', 'regular selling amount', 
