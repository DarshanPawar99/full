...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_pax = max(safe_get_value(row, 'ordered pax/vendor mg'),safe_get_value(row, 'vendor actual consumption'))
...
def calculate_aggregated_values(df_filtered):
    ...
    regular_and_adhoc_orders = df_filtered[df_filtered['order type'].isin(['regular', 'smartq-pop-up', 'food trial', 'regular-pop-up','rent'])]
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    st.write("Buying on Higest of Ordered Pax, Vendor Actual Consumption")
    st.write("Selling  on Higest of Client MG, Actual Consumption")
    st.markdown("---")
...
def business_logic_36(df_selected_month, analysis_df, months_to_include):
