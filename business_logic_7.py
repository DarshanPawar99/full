...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            meal_type = safe_get_value(row, 'meal type')
            if meal_type in ['buffet', 'packed']:
                calculated_selling_pax = safe_get_value(row, 'client dc cosumption')
            elif meal_type == 'saladbar':
                calculated_selling_pax = safe_get_value(row, 'buying pax')
            else:
                logging.error(f"Unknown meal type '{meal_type}' in row {index + 3}")
                continue
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    ...
    st.write("For Buffet and Packed Selling  on Client DC Cosumption ")
    st.write("for Saladbar Selling  on Buying Pax")
    st.markdown("---")
...
def business_logic_7(df_selected_month, analysis_df, months_to_include):
