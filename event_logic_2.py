...
def pivot_and_average_prices(df_filtered):
    ...
    combined_df = df_filtered.pivot_table(
        index=['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price'],
        aggfunc='size'
    ).reset_index(name='days')

    total_pax = df_filtered.groupby(['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price']).agg(
        total_buying_pax=('buying pax', 'sum'),
        total_selling_pax=('selling pax', 'sum')
    ).reset_index()
    
    pivot_df = combined_df.merge(total_pax, on=['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price'])
    return pivot_df
...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            calculated_buying_price = (safe_get_value(row, 'buying price') * safe_get_value(row, 'gst'))+safe_get_value(row, 'buying price')
...
def find_karbon_expenses(df_filtered):
    ...
    columns_to_check = ['date(karbon)','site name','expense item', 'reason for expense', 'expense type', 'amount', 'mode of payment','bill to','requested by','approved by']
    for index, row in df_filtered.iterrows():
...
def event_logic_2(df_selected_month, analysis_df, months_to_include):
