    ...
    for index, row in df_filtered.iterrows():
        try:
           # for buying price
            ...
            buying_mg_pax = safe_get_value(row, 'buying price ai')

            if meal_type == "breakfast":
                if buying_mg_pax <= 200:
                    buying_price = 62
                ...
                if buying_mg_pax <= 100:
                    buying_price = 49
                elif buying_mg_pax <= 200:
                    buying_price = 48
                else:
                    buying_price = 47
            else:
                buying_price = None  # Handle undefined meal types if necessary

            check_mismatch(row, index, 'buying price ai', buying_price, mismatched_data)
            ...
            staff_cab = safe_get_value(row, 'cab driver')

            if (buying_mg_pax + actual_consumption + partners_sales + manual_entry + training_staff + staff_cab) < actual_consumption:
                calculated_delta_pax = actual_consumption - (buying_mg_pax + actual_consumption + partners_sales + manual_entry + training_staff + staff_cab)
            else:
            ...
            total_pax_buying = max((buying_mg_pax + actual_consumption + partners_sales + manual_entry + training_staff), safe_get_value(row, 'buying mg/pax'))

            check_mismatch(row, index, 'total pax buying', total_pax_buying, mismatched_data)

            # Buying amount only for breakfast 
            if row['meal type (only lunch)'] == 'breakfast':
                ...
                # Buying amount for non-breakfast meals
            ...
            actual_consumption = safe_get_value(row, 'actual consumption/employee')
            partners_sales = safe_get_value(row, 'training new joining  staff btc')
            manual_entry = safe_get_value(row, 'manual entry')

            calculated_delta_pax_btc = max(selling_mg_pax_btc - (actual_consumption + manual_entry), partners_sales, 0)
            ...
            partners_sales = safe_get_value(row, 'partners(direct cash sales)')
            employee_50_percent = safe_get_value(row, 'selling price')

            if row['meal type (only lunch)'] in ['breakfast']:
                ...
                partners_sales_amount_employee_50_percent = ((partners_sales * employee_50_percent)*2) + ((actual_consumption + manual_entry) *employee_50_percent)
                check_mismatch(row, index, 'partners(direct cash sales) amount+employee 50%', partners_sales_amount_employee_50_percent, mismatched_data)
            



            #  total sales only for breakfast 
            if row['meal type (only lunch)'] in ['breakfast']:
                selling_amount = (safe_get_value(row, 'manual entry') * safe_get_value(row, 'selling price')) + (safe_get_value(row, 'delta pax(gap between mg and consumption) btc') * safe_get_value(row, 'selling price')) + safe_get_value(row, 'partners(direct cash sales) amount+employee 50%')
                check_mismatch(row, index, 'total sales', selling_amount, mismatched_data)
            else:
                selling_amount = (safe_get_value(row, 'manual entry') * safe_get_value(row, 'selling price')) + (safe_get_value(row, 'delta pax(gap between mg and consumption) btc') * safe_get_value(row, 'selling price') *2 ) + safe_get_value(row, 'partners(direct cash sales) amount+employee 50%') 
                check_mismatch(row, index, 'total sales', selling_amount, mismatched_data)
            ...
            #  total sales only for breakfast 
            if row['meal type (only lunch)'] in ['breakfast']:
                bill_to_client = ((safe_get_value(row, 'actual consumption/employee') + safe_get_value(row, 'manual entry')) * safe_get_value(row, 'selling price')) + (safe_get_value(row, 'delta pax(gap between mg and consumption) btc') * safe_get_value(row, 'selling price'))
                check_mismatch(row, index, 'bill to client', bill_to_client, mismatched_data)
            else:
                bill_to_client = ((safe_get_value(row, 'actual consumption/employee') + safe_get_value(row, 'manual entry')) * safe_get_value(row, 'selling price')) + (safe_get_value(row, 'delta pax(gap between mg and consumption) btc') * safe_get_value(row, 'selling price') *2 ) 
                check_mismatch(row, index, 'bill to client', bill_to_client, mismatched_data)
...
def analysis_data(analysis_df, months_to_include):
    ...
    columns_to_fill = ['total pax selling', 'total pax buying', 'bill to client', 'comission', 'buying price ai']
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_selling_amt_regular = df_filtered['bill to client'].sum()
    sum_cash_recived = df_filtered['partners(direct cash sales) amount+employee 50%'].sum()
...
def display_dataframes(pivot_df, mismatched_data, aggregated_data, summary_df):
    st.write("This site as Training New Joining Staff & Cab Driver")
...
def business_logic_48(df_selected_month, analysis_df, months_to_include):
    ...
    display_dataframes(pivot_df, mismatched_data, aggregated_data, summary_df)
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                ...
                'regular selling amount': [full_data['bill to client'].sum()],
                'cash recevied' : [full_data['partners(direct cash sales) amount+employee 50%'].sum()],
               })
...
def dump_data(df_filtered, month, dump_file_path):
    dump_mapping = {
        ...
        'cash recived' : 'partners(direct cash sales) amount+employee 50%'
    }
