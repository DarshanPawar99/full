...
def pivot_and_average_prices(df_filtered):
    ...
    combined_df = df_filtered.pivot_table(
        index=['site', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price'],
        aggfunc='size'
    ).reset_index(name='days')

    total_pax = df_filtered.groupby(['site', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price']).agg(
        total_buying_pax=('buying pax', 'sum'),
        total_selling_pax=('selling pax', 'sum')
    ).reset_index()
    
    pivot_df = combined_df.merge(total_pax, on=['site', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price'])
    return pivot_df
...
def event_logic_12(df_selected_month, analysis_df, months_to_include):
