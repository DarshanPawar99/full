...
def pivot_and_average_prices(df_filtered):
    ...
    pivot_df = df_filtered.pivot_table(
        index=['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type','buying pax'],
        aggfunc='size'
...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_amt = safe_get_value(row, 'buying pax') * safe_get_value(row, 'buying price ai') 
            check_mismatch(row, index, 'discounted buying amt ai', calculated_buying_amt, mismatched_data)

            calculated_selling_amount = safe_get_value(row, 'discounted buying amt ai') + (safe_get_value(row, 'discounted buying amt ai') * safe_get_value(row, 'discount%'))
            check_mismatch(row, index, 'actual mrp', calculated_selling_amount, mismatched_data)

            calculated_support = safe_get_value(row, 'actual mrp') * 0.07
            check_mismatch(row, index, 'selling management fee', calculated_support, mismatched_data)

            calculated_buying_amt = safe_get_value(row, 'actual mrp') + safe_get_value(row, 'selling management fee') 
            check_mismatch(row, index, 'selling amount', calculated_buying_amt, mismatched_data)
            
            calculated_commission = safe_get_value(row, 'selling amount') - safe_get_value(row, 'discounted buying amt ai')
...
def analysis_data(analysis_df, months_to_include):
    ...
    columns_to_fill = ['selling amount', 'commission', 'discounted buying amt ai']
    analysis_df[columns_to_fill] = analysis_df[columns_to_fill].fillna(0)

    analysis_df['total_gmv'] = analysis_df['selling amount']
    analysis_df['total_buying'] = analysis_df['discounted buying amt ai']
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_buying_amt_ai_regular= df_filtered['discounted buying amt ai'].sum()
    ...
    for index, row in df_filtered.iterrows():
        if (safe_get_value(row, 'discounted buying amt ai') > safe_get_value(row, 'selling amount') > 0):
            high_buying.append({
                ...
                'Buying Amount AI': row['discounted buying amt ai'],
...
def business_logic_8(df_selected_month, analysis_df, months_to_include):
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                ...
                'regular buying amount': [full_data['discounted buying amt ai'].sum()],
...
def dump_data(df_filtered, month, dump_file_path):
    dump_mapping = {
        ...
        'buying amt ai': 'discounted buying amt ai',
    ...
    try:
        ...
        if 'selling management fee1' in df_filtered.columns:
            ...
            for site_name, group in grouped:
                selling_sum = group['selling management fee1'].sum()
