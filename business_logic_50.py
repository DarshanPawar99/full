...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_selling_price = safe_get_value(row, 'selling price with  commission')
            check_mismatch(row, index, 'selling price', calculated_selling_price, mismatched_data)
            

            calculated_selling_price_with_commission = safe_get_value(row, 'mrp') * 1.08
            check_mismatch(row, index, 'selling price with  commission', calculated_selling_price_with_commission, mismatched_data)
...
def business_logic_50(df_selected_month, analysis_df, months_to_include):
