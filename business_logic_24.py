...
def pivot_and_average_prices(df_filtered):
    ...
    pivot_df = df_filtered.pivot_table(
        index=['site name', 'vendor', 'session','meal type', 'order type','unit price'],
        aggfunc='size'
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_buying_pax_regular = df_filtered['fruit qty'].sum()
    sum_selling_pax_regular = df_filtered['fruit qty'].sum()
    ...
    valid_dates_df = df_filtered[(df_filtered['fruit qty'] > 0)]
...
def business_logic_24(df_selected_month, analysis_df, months_to_include):
...
def load_business_logic(df_filtered):
    try:
        ...
        for cost_centre, group in grouped:
            ...
            no_of_days = group[(group['fruit qty'] > 0)]

            pnl_data = pd.DataFrame({
                'days': [no_of_days['date'].nunique()],
                'buying pax': [full_data['fruit qty'].sum()],
                'selling pax': [full_data['fruit qty'].sum()],
        ...
        for key_value, group in df_filtered.groupby('identifier'):
            ...
            pnl_df.loc[(pnl_df['identifier'] == key_value) & (pnl_df['month'] == month), 
                       ['days', 'buying pax', 'selling pax', 'regular buying amount', 'regular selling amount', 
                         'sams']] = pnl_data[['days', 'buying pax', 'selling pax', 'regular buying amount', 'regular selling amount', 
                        'sams']].values
