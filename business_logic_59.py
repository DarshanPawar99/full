...
def analysis_data(analysis_df, months_to_include):
    ...
    analysis_df['regular_gmv'] = analysis_df.apply(lambda x: x['selling amount'] if x['order type'] in regular_types else 0, axis=1)
    analysis_df['event_gmv'] = analysis_df.apply(lambda x: x['selling amount'] if x['order type'] in event_types else 0, axis=1)

    # Calculate additional columns
    analysis_df['paxs_difference'] = analysis_df['selling pax'] - analysis_df['buying pax']
    ...
    metrics = ['regular_gmv', 'event_gmv', 'commission', 'karbon', 'paxs_difference', 'pd_revenue', 'net_revenue', 'total_gmv']
    grouped_df = analysis_df.groupby('month')[metrics].sum().reset_index()

    # Calculate pd_revenue_percentage
    grouped_df['pd_revenue_percentage'] = grouped_df.apply(lambda row: (row['pd_revenue'] / row['commission']) * 100 if row['commission'] != 0 else 0, axis=1)
    grouped_df['pd_revenue_percentage'] = grouped_df['pd_revenue_percentage'].round(2)

    # Calculate event_gmv_percentage
    grouped_df['event_gmv_percentage'] = grouped_df.apply(lambda row: (row['event_gmv'] / row['total_gmv']) * 100 if row['total_gmv'] != 0 else 0, axis=1)
    grouped_df['event_gmv_percentage'] = grouped_df['event_gmv_percentage'].round(2)
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_selling_management = df_filtered['selling management fee'].sum()
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df):
    st.write("Buying on Ordered Pax/Vendor MG")
    st.write("Selling on Client MG")
    st.markdown("---")
...
def business_logic_59(df_selected_month, analysis_df, months_to_include):
    ...
    popup_selling_issues = find_popup_selling_issues(df_filtered)
    high_buying = find_higher_buying(df_filtered)
    ...
    display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, 
                       buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df)
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                ...
                'support on sells': [full_data['selling management fee'].sum()]
            })
...
def process_data(pnl_df, pnl_data):
    try:
        ...
        pnl_mapping = {
            ...
            'sams': 'sams',
            'support on sells': 'support on sells'
        }
        ...
        for key_value, group in df_filtered.groupby('identifier'):
            ...
            pnl_df.loc[(pnl_df['identifier'] == key_value) & (pnl_df['month'] == month), 
                       ['days', 'buying pax', 'selling pax', 'regular buying amount', 'regular selling amount', 
                        'event buying amount', 'event selling amount', 'penalty on vendor', 'penalty on smartq', 'sams','support on sells' ]] = pnl_data[['days', 'buying pax', 'selling pax', 'regular buying amount', 'regular selling amount', 
                        'event buying amount', 'event selling amount', 'penalty on vendor', 'penalty on smartq', 'sams','support on sells']].values
...
def clear_pnl_data(p_and_l_file_path, month):
    ...
    try:
        ...
        columns_to_clear = [
            ...
            'event selling amount', 'penalty on vendor', 'penalty on smartq', 'sams','support on sells'
        ]
