...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_pax = max(safe_get_value(row, 'vendor mg'), safe_get_value(row, 'actual consumption vendor'))
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    st.write("Buying on Highest among Vendor MG, Vendor Actual Consumption")
    st.write("Selling on Highest among Client MG, Client Actual Consumption")
...
def business_logic_64(df_selected_month, analysis_df, months_to_include):
