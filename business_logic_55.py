...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            # Check for direct payment from employee
            if row['menu item'] in ['order on the go']:
                calculated_cash_recived = safe_get_value(row, 'pax sold') * safe_get_value(row, 'selling price')
                check_mismatch(row, index, 'direct payment from employee', calculated_cash_recived, mismatched_data)


            calculated_buyinging_pax = safe_get_value(row, 'ordered pax/vendor mg')
            ...
            check_mismatch(row, index, 'buying pax', calculated_buyinging_pax, mismatched_data)
            ...
            # Check for filled buying pax and amount in breakfast and snacks
            if row['menu item'] in ['order on the go']:
                if row['buying pax'] != 0 or row['selling pax'] != 0 or row['buying amt ai'] != 0 or row['selling amount'] != 0:
                    price_in_ong.append({
                        'Row': index + 3,
                        'Date': row['date'],
                        'Session': row['session'],
            ...
            # Check for missing buying pax and amount in veg lunch and non-veg lunch
            if row['menu item'] in ['support staff']:
                if pd.isna(row['buying amt ai']):
                    price_in_ss.append({
                        'Row': index + 3,
                        'Date': row['date'],
                        'Session': row['session'],
...
def calculate_aggregated_values(df_filtered):
    regular_orders = df_filtered[df_filtered['order type'] .isin(['regular','regular-pop-up', 'food trial','support staff'])]
    sum_buying_pax_regular = regular_orders['buying pax'].sum()
    sum_selling_pax_regular = regular_orders['selling pax'].sum()

    regular_and_adhoc_orders = df_filtered[df_filtered['order type'].isin(['regular', 'smartq-pop-up', 'food trial', 'regular-pop-up','support staff'])]
...
def find_buying_value_issues(df_filtered):
    buying_value_issues = []
    for index, row in df_filtered.iterrows():
        if row['menu item'] in ['veg & non veg meal']:
...
def find_selling_value_issues(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        if row['order type'] in ['veg & non veg meal','support staff']:
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    ...
    st.write("Selling on Highest among Client MG, vendor MG, Actual Consumption.")
...
def business_logic_55(df_selected_month, analysis_df, months_to_include):
