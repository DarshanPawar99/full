...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            # Additional checks for 'snacks' in 'session'
            if row['order type'] .isin(['regular-pop-up', 'event-pop-up', 'smartq-pop-up']):
                calculated_selling_pax = 0
            else:
...
def business_logic_9(df_selected_month, analysis_df, months_to_include):
