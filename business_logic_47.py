    ...
    for index, row in df_filtered.iterrows():
        try:
          ...
          # for delta pax(gap between mg and consumption)
            ...
            if row['meal type (only lunch)'] in ['breakfast']:
                total_sales = ((safe_get_value(row, 'actual consumption/employee') + safe_get_value(row, 'manual entry')) * safe_get_value(row, 'selling price')) + (safe_get_value(row, 'delta pax(gap between mg and consumption) btc') * safe_get_value(row, 'selling price')) + safe_get_value(row, 'partners(direct cash sales) amount')
                check_mismatch(row, index, 'total sales', total_sales, mismatched_data)
            else:
                total_sales = ((safe_get_value(row, 'actual consumption/employee') + safe_get_value(row, 'manual entry')) * safe_get_value(row, 'selling price')) + (safe_get_value(row, 'delta pax(gap between mg and consumption) btc') * safe_get_value(row, 'selling price')*2 ) + safe_get_value(row, 'partners(direct cash sales) amount')
                check_mismatch(row, index, 'total sales', total_sales, mismatched_data)
            ...
            if row['meal type (only lunch)'] in ['breakfast']:
                ...
                btc = ((actual_consumption + manual_entry) * selling_price) + (partners_sales * selling_price)*2
                check_mismatch(row, index, 'btc', btc, mismatched_data)
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_cash_recived = df_filtered['partners(direct cash sales) amount'].sum()
...
def display_dataframes(pivot_df, mismatched_data, aggregated_data, summary_df, session_wise_df):
    st.write("This site as Training New Joining Staff & Gym Trainer ")
...
def business_logic_47(df_selected_month, analysis_df, months_to_include):
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                ...
                'cash recevied' : [full_data['partners(direct cash sales) amount'].sum()],
               })
...
def dump_data(df_filtered, month, dump_file_path):
    dump_mapping = {
        ...
        'cash recived' : 'partners(direct cash sales) amount'
    }
