...
def calculate_aggregated_values(df_filtered):
    ...
    non_curd_orders = regular_orders[~regular_orders['menu item'].isin(['curd'])]
    sum_buying_pax_regular = non_curd_orders['buying pax'].sum()
    sum_selling_pax_regular = non_curd_orders['selling pax'].sum()
...
def business_logic_56(df_selected_month, analysis_df, months_to_include):
