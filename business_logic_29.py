...
def pivot_and_average_prices(df_filtered):
    ...
    combined_df = df_filtered.pivot_table(
        index=['site name', 'vendor', 'session','event name', 'meal type','order type', 'buying price ai', 'selling price','remarks'],
        aggfunc='size'
    ).reset_index(name='days')

    total_pax = df_filtered.groupby(['site name', 'vendor', 'session','event name', 'meal type','order type', 'buying price ai', 'selling price','remarks']).agg(
        total_buying_pax=('buying pax', 'sum'),
        total_selling_pax=('selling pax', 'sum')
    ).reset_index()
    
    pivot_df = combined_df.merge(total_pax, on=['site name', 'vendor', 'session','event name', 'meal type','order type', 'buying price ai', 'selling price','remarks'])
    return pivot_df
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            if session == "sales force - gur":
                ...
                calculated_buying_pax = "New Site Name"
                check_mismatch(row, index, 'buying pax', calculated_buying_pax, mismatched_data)
            ...
            if session == "sales force - gur":
                ...
                calculated_selling_pax = "New Site Name"
                check_mismatch(row, index, 'selling management fee', calculated_selling_pax, mismatched_data)
...
def business_logic_29(df_selected_month, analysis_df, months_to_include):
    ...
    session_wise_df = pd_session_wise(df_filtered)
