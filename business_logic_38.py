...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            check_mismatch(row, index, 'buying amt regular', calculated_buying_amt, mismatched_data)

            calculated_buying_amt = (safe_get_value(row, 'buying price ai') * safe_get_value(row, 'buying pax') 
                                     + safe_get_value(row, 'buying transportation')+safe_get_value(row, 'buying manpower'))
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    ...
    st.write("Man-Power is Paid to Vendor")
    st.markdown("---")
...
def business_logic_38(df_selected_month, analysis_df, months_to_include):
...
def dump_data(df_filtered, month, dump_file_path):
    ...
    try:
        ...
        if 'buying manpower' in df_filtered.columns:
            ...
            for site_name, group in grouped:
                buying_sum = group['buying manpower'].sum()
                
                # Create a new row for each group with the aggregated data
                new_row = pd.DataFrame({
                    'month': [month],
                    'site name': [site_name],
                    'order type': ['manpower fee'],
                    'selling pax': 1,
                    'buying price ai': [buying_sum],
                    'buying amt ai': [buying_sum]
                })
        ...
        if 'selling management fee' in df_filtered.columns:
            ...
            for site_name, group in grouped:
                buying_sum = group['selling management fee'].sum()
                
                # Create a new row for each group with the aggregated data
                new_row = pd.DataFrame({
                    'month': [month],
                    'site name': [site_name],
                    'order type': ['manpower cost'],
                    'buying pax': 1,
                    'selling price': [buying_sum],
                    'selling amount': [buying_sum]
                })
