...
def pivot_and_average_prices(df_filtered):
    ...
    pivot_df = df_filtered.pivot_table(
        index=['site name', 'session', 'names','designation', 'order type','buying amt ai', 'selling amount','remarks'],
        aggfunc='size'
...
def display_dataframes(pivot_df, mismatched_data, buying_df, selling_df, buying_value_issues, selling_value_issues, double_entry, summary_df):
    st.subheader("")
    st.subheader("Well-Frgo Other Revenues Data")
...
def welfrgo_other_revenues(df_selected_month, analysis_df, months_to_include):
    
    # Sidebar multiselect for site name
    selected_sites = st.sidebar.multiselect("Select City Name(s)", df_selected_month['city'].unique())

    # Filter dataframe by selected sites
    if selected_sites:
        df_filtered = df_selected_month[df_selected_month['city'].isin(selected_sites)]
    else:
    ...
    if selected_sites:
        analysis_df = analysis_df[analysis_df['city'].isin(selected_sites)]
    else:
