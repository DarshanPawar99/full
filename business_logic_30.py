...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_pax = max(safe_get_value(row, 'client mg/pre order'),safe_get_value(row, 'actual consumption')) 
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    st.write("Buying is Highest Client MG or Actual Consumption")
    st.write("Selling is Same as Buying")
    st.markdown("---")
...
def business_logic_30(df_selected_month, analysis_df, months_to_include):
