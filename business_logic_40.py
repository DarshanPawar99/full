...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            calculated_buying_pax = safe_get_value(row, 'company paid') + safe_get_value(row, 'contract employees')
            ...
            calculated_selling_amount = safe_get_value(row, 'contract employees') * safe_get_value(row, 'selling price')
            check_mismatch(row, index, 'direct payment from employee', calculated_selling_amount, mismatched_data)
...
def calculate_aggregated_values(df_filtered):
    ...
    regular_and_adhoc_orders = df_filtered[df_filtered['order type'].isin(['regular','smartq-pop-up', 'food trial', 'regular-pop-up','tuckshop','live'])]
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    st.write("Buying on Company Paid + Contract Employees.")
    st.write("Selling is same Buying.")
...
def business_logic_40(df_selected_month, analysis_df, months_to_include):
