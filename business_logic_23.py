...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            calculated_buying_price = safe_get_value(row, 'buying price ai') / (safe_get_value(row, 'gst') if safe_get_value(row, 'gst') != 0 else 1)
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df):
    st.write("Buying on Ordered MG")
    st.write("Selling on Actual Consumption")
    st.markdown("---")
...
def business_logic_23(df_selected_month, analysis_df, months_to_include):
