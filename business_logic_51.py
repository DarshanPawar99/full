...
def pivot_and_average_prices(df_filtered):
    ...
    total_pax = df_filtered.groupby(['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price', 'remarks']).agg(
        total_buying_pax=('buying pax', 'sum'),
        total_selling_pax=('selling pax', 'sum')
    ).reset_index()
    
    pivot_df = combined_df.merge(total_pax, on=['site name', 'vendor', 'session', 'menu item', 'meal type', 'order type', 'buying price ai', 'selling price', 'remarks'])
    return pivot_df
...
def analysis_data(analysis_df, months_to_include):
    ...
    columns_to_fill = ['amount', 'selling pax', 'buying pax', 'selling amt', 'commission', 'buying price ai']
    ...
    analysis_df['regular_gmv'] = analysis_df['selling amt'].where(analysis_df['order type'].isin(regular_types), 0)
    analysis_df['event_gmv'] = analysis_df['selling amt'].where(analysis_df['order type'].isin(event_types), 0)
    ...
    analysis_df['total_gmv'] = analysis_df['selling amt']
...
def find_mismatches(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        try:
            ...
            check_mismatch(row, index, 'buying amt total', calculated_buying_amt, mismatched_data)

            calculated_buying_pax = safe_get_value(row, 'client mg/pre order') - safe_get_value(row, 'actual consumption')
            check_mismatch(row, index, 'selling pax', calculated_buying_pax, mismatched_data)

            calculated_buying_pax = safe_get_value(row, 'ordered pax/vendor mg') - safe_get_value(row, 'actual consumption')
            ...
            check_mismatch(row, index, 'selling amt', calculated_selling_amount, mismatched_data)
            
            calculated_commission = (safe_get_value(row, 'selling amt') - safe_get_value(row, 'buying amt total') 
    ...
    for index, row in df_filtered.iterrows():
        if any(pd.notna(row[col]) and row[col] != 0 for col in columns_to_check):
            karbon_expenses_data.append({
                'Row': index + 3,
                'Buying Amount': row['buying amt total'],
...
def calculate_aggregated_values(df_filtered):
    ...
    sum_buying_amt_ai_regular= regular_and_adhoc_orders['buying amt total'].sum()
    sum_selling_amt_regular = regular_and_adhoc_orders['selling amt'].sum()

    event_and_popup_orders = df_filtered[df_filtered['order type'].isin(['event', 'event-pop-up', 'adhoc'])]
    sum_buying_amt_ai_event= event_and_popup_orders['buying amt total'].sum()
    sum_selling_amt_event = event_and_popup_orders['selling amt'].sum()
...
def find_buying_value_issues(df_filtered):
    buying_value_issues = []
    for index, row in df_filtered.iterrows():
        if (safe_get_value(row, 'buying pax') > 0 or safe_get_value(row, 'buying price ai') > 0) and safe_get_value(row, 'buying amt total') == 0:
            buying_value_issues.append({
                ...
                'Buying Amount AI': row['buying amt total']
            })
...
def find_higher_buying(df_filtered):
    high_buying = []
    for index, row in df_filtered.iterrows():
        if (safe_get_value(row, 'buying pax') > safe_get_value(row, 'selling pax') > 0) or (safe_get_value(row, 'buying amt total') > safe_get_value(row, 'selling amt') > 0):
            high_buying.append({
                'Row': index + 3,
                'Date': row['date'],
                'Session': row['session'],
                ...
                'Buying Amount AI': row['buying amt total'],
                'Selling Amount': row['selling amt'],
                'Remarks': row['remarks']
            })
    return high_buying
...
def find_selling_value_issues(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        if (safe_get_value(row, 'selling pax') > 0 or safe_get_value(row, 'selling price') > 0) and safe_get_value(row, 'selling amt') == 0:
...
def find_popup_selling_issues(df_filtered):
    ...
    for index, row in df_filtered.iterrows():
        if row['order type'] in ['smartq-pop-up', 'regular-pop-up', 'event-pop-up'] and safe_get_value(row, 'selling amt') > 0:
...
def display_dataframes(pivot_df, mismatched_data, karbon_expenses_data, aggregated_data, buying_value_issues, selling_value_issues, popup_selling_issues, high_buying, summary_df, session_wise_df):
    ...
    st.write("Tea Counter is also present")
    st.markdown("---")
...
def business_logic_51(df_selected_month, analysis_df, months_to_include):
    ...
    try:
        ...
        for cost_centre, group in grouped:
            ...
            pnl_data = pd.DataFrame({
                ...
                'regular buying amount': [regular_amt['buying amt total'].sum()],
                'regular selling amount': [regular_amt['selling amt'].sum()],
                'event buying amount': [event_amt['buying amt total'].sum()],
                'event selling amount': [event_amt['selling amt'].sum()],
...
def dump_data(df_filtered, month, dump_file_path):
    dump_mapping = {
        ...
        'buying amt ai': 'buying amt total',
        'selling pax': 'selling pax',
        'selling price': 'selling price',
        'selling transportation': 'selling transportation',
        'selling amount': 'selling amt',
        'penalty on vendor': 'penalty on vendor',
        'penalty on smartq': 'penalty on smartq',
